...
public class BPlusTree {
    ...
    public Optional<RecordId> get(DataBox key) {
        ...
        LeafNode leaf = root.get(key); // start from the root to find the leafnode.
        return leaf.getKey(key); // return the record in this leafnode.
    }
    ...
    public Iterator<RecordId> scanAll() {
        ...
        // start with the leftmostLeafNode
        LeafNode leftLeafNode = root.getLeftmostLeaf();
        return new BPlusTreeIterator(leftLeafNode);
    }
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        Optional<Pair<DataBox, Long>> rootPutResult = root.put(key, rid);
        // if root split
        if (rootPutResult.isPresent()) {
            DataBox new_key = rootPutResult.get().getFirst();
            Long new_children = rootPutResult.get().getSecond();
            // create a new_root
            ArrayList<DataBox> new_root_keys = new ArrayList<>(Arrays.asList(new_key));
            ArrayList<Long> new_root_children = new ArrayList<>(Arrays.asList(root.getPage().getPageNum(), new_children));
            BPlusNode new_root = new InnerNode(metadata, bufferManager, new_root_keys, new_root_children, lockContext);
            updateRoot(new_root);
        }
    }
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        //Raise an exception if the tree is not empty at time of bulk loading
        if (!(root instanceof LeafNode) || !((LeafNode) root).getKeys().isEmpty()) {
            throw new BPlusTreeException("bulk loading can only use on an empty tree");
        }
        ...
        while (data.hasNext()) {
            ...
            if (bulkresult.isPresent()) {
                DataBox new_key = bulkresult.get().getFirst();
                Long new_children = bulkresult.get().getSecond();
                // create a new_root
                ArrayList<DataBox> new_root_keys = new ArrayList<>(Arrays.asList(new_key));
                ArrayList<Long> new_root_children = new ArrayList<>(Arrays.asList(root.getPage().getPageNum(), new_children));
                BPlusNode new_root = new InnerNode(metadata, bufferManager, new_root_keys, new_root_children, lockContext);
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        BPlusTreeIterator(LeafNode leafNode) {
            ...
            recordIdIterator = currLeafNode.scanAll();
        }

        /**
         This is for scanGreaterEqual.
         */
        BPlusTreeIterator(LeafNode leafNode, DataBox key) {
            currLeafNode = leafNode;
            recordIdIterator = currLeafNode.scanGreaterEqual(key);
        }

        @Override
        public boolean hasNext() {
            // DONE(proj2): implement
            // if currentLeafNode still has next, or it has rightSibling
            return recordIdIterator.hasNext() || currLeafNode.getRightSibling().isPresent();
        }
