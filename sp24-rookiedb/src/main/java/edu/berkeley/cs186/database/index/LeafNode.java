...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        // DONE(proj2): implement
        // if the key already exists in the B+ tree, then the pair is not inserted and an exception is raised.
        if (getKey(key).isPresent()) {
            String msg = String.format("already contains this key: %s", key);
        ...
        if (keys.size() > 2*d) {
            ...
            ArrayList<DataBox> new_keys = new ArrayList<>(keys.subList(d, keys.size()));
            ArrayList<RecordId> new_rids = new ArrayList<>(rids.subList(d, rids.size()));
            ...
            Long new_leafNode_pageNum = new_leafnode.getPage().getPageNum();
            this.rightSibling = Optional.of(new_leafNode_pageNum);
            //ensure that representation of a node in our buffers is up-to-date with the representation of the node in program memory.
            sync();
            // pair (split_key, right_node_page_num) is returned.
            return Optional.of(new Pair<>(split_key, new_leafNode_pageNum));
        }
        //ensure that representation of a node in our buffers is up-to-date with the representation of the node in program memory.
        sync();
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        // DONE(proj2): implement
        int d = metadata.getOrder();
        int maxFillSize = (int) Math.ceil(2*d*fillFactor);
        // added sorted pairs until no more data left or overflow.
        while (keys.size() <= maxFillSize && data.hasNext()) {
            Pair<DataBox, RecordId> new_pair = data.next();
            DataBox new_key = new_pair.getFirst();
            RecordId new_rid = new_pair.getSecond();
            keys.add(new_key);
            rids.add(new_rid);
        }
        if (keys.size() > maxFillSize) {
            //find the split key
            DataBox split_key = keys.get(maxFillSize);
            //split the leafNode into two.
            ArrayList<DataBox> new_keys = new ArrayList<>(keys.subList(maxFillSize, keys.size()));
            ArrayList<RecordId> new_rids = new ArrayList<>(rids.subList(maxFillSize, rids.size()));
            LeafNode new_leafnode = new LeafNode(metadata, bufferManager, new_keys, new_rids, rightSibling, treeContext);
            keys = keys.subList(0,maxFillSize);
            rids = rids.subList(0,maxFillSize);
            Long new_leafNode_pageNum = new_leafnode.getPage().getPageNum();
            this.rightSibling = Optional.of(new_leafNode_pageNum);
            //ensure that representation of a node in our buffers is up-to-date with the representation of the node in program memory.
            sync();
    ...
    public void remove(DataBox key) {
        ...
        // dose nothing if the key k is not in the subtree.
        if (!getKey(key).isPresent()) {
            return;
        }
        int removeIndex = InnerNode.numLessThan(key, keys);
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        // brand new one. **Using functions with page**.
        Page page = bufferManager.fetchPage(treeContext, pageNum);
        Buffer buf = page.getBuffer();

        byte nodeType = buf.get();
        assert (nodeType == (byte) 1); //the literal value 1 (1 byte) which indicates that this node is leaf node,

        List<DataBox> keys = new ArrayList<>();
        List<RecordId> rids = new ArrayList<>();
        Long r = buf.getLong();
        // the page id (8 bytes) of our right sibling (or -1 if we don't have)
        Optional<Long> rightSibling = Optional.ofNullable((r==-1) ? null : r);

        int n = buf.getInt();// n is the number of (key, rid) pairs
        for (int i = 0; i < n; ++i) {
