...
class InnerNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        int i  = numLessThanEqual(key, keys);
        BPlusNode child = getChild(i);
        // recursively do get and use the get result
        Optional<Pair<DataBox, Long>> childPutResult = child.put(key, rid);

        // if the child overflowed.
        if (childPutResult.isPresent()) {
            DataBox new_key = childPutResult.get().getFirst();
            Long right_node_page_num = childPutResult.get().getSecond();
            int d = metadata.getOrder();
            // insert the new_key and new_child_node.
            int insetIndex = numLessThanEqual(new_key, keys);
            keys.add(insetIndex, new_key);
            children.add(insetIndex+1, right_node_page_num);
            // if overflow
            if (keys.size() > 2*d) {
                //find the split key
                DataBox split_key = keys.get(d);
                //split the innerNode into two, not keep the split_key.
                ArrayList<DataBox> new_keys = new ArrayList<>(keys.subList(d+1, keys.size()));
                ArrayList<Long> new_children = new ArrayList<>(children.subList(d+1, children.size()));
                InnerNode new_innerNode = new InnerNode(metadata, bufferManager, new_keys, new_children, treeContext);
                Long new_innerNode_pageNum = new_innerNode.getPage().getPageNum();
                ...
                return Optional.of(new Pair<>(split_key, new_innerNode_pageNum));
            }
            //ensure that representation of a node in our buffers is up-to-date with the representation of the node in program memory.
            sync();
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        while (keys.size()<=2*d && data.hasNext()) {
            BPlusNode rightChild = this.getChild(keys.size());
            Optional<Pair<DataBox, Long>> childBulkResult = rightChild.bulkLoad(data, fillFactor);
            // if rightmost child overflowed
            if (childBulkResult.isPresent()) {
               DataBox new_key = childBulkResult.get().getFirst();
               Long new_childNode_page_num = childBulkResult.get().getSecond();
               // update the keys and children list.
               keys.add(new_key);
               children.add(new_childNode_page_num);
            }
        }
        // if this innerNode overflow
        if (keys.size() > 2*d) {
            //find the split key
            DataBox split_key = keys.get(d);
            //split the innerNode into two, not keep the split_key.
